...
class XSystem35Activity : EngineActivity() {
    ...
    // Белый список окон оставлен как ручная настройка, но по умолчанию пуст:
    // игра переиспользует номера окон (1/2 — то диалог, то бой/меню), поэтому
    // отбор «что читать» делает движок — озвучивается только бокс, на котором
    // игра остановилась в ожидании клика (sys_hit_any_key).
    override fun applyReadWindows(csv: String) {
        NativeBridge.setReadWindows(csv)
    }

    // Диалог/бой/меню в System 3.9 поведенчески неразличимы (скрипт сам
    // опрашивает ввод), но живут на разных страницах сценария. Для Daiakuji:
    // 11 — хаб/титул (статус «35Y 1M4W…»), 19 — бой (лог действий).
